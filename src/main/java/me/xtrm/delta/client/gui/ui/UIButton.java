...
public class UIButton extends GuiButton {
	...
	public UIButton(int id, int x, int y, int width, int height, String text) { super(id, x, y, width, height, text); alpha = 80; }
	public UIButton(int id, int x, int y, String text) { super(id, x, y, text); alpha = 80; }

	@Override
	public void drawButton(Minecraft mc, int mouseX, int mouseY) {
		if(this.visible) {
			if(!this.enabled) {
				GuiUtils.getInstance().drawRect(xPosition, yPosition, xPosition + width, yPosition + height, 0xDE000000);
				...
				GuiUtils.getInstance().drawRect(xPosition, yPosition, xPosition + width, yPosition + height, color.getRGB());
				Fonts.font.drawStringWithShadow(
						displayString, 
						xPosition + (width / 2) - (Fonts.font.getStringWidth(displayString) / 2), 
						yPosition + (height / 2) - (Fonts.font.getFontHeight() / 2), 
